...
export function HostApplications() {
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-center gap-3"><span className="text-sm text-stone">{t('filterByGathering')}:</span><Select value={gatheringFilter} onValueChange={(v) => setParam('gathering', v)}><SelectTrigger className="w-[20rem] bg-card" size="sm"><SelectValue placeholder={t('allGatherings')} /></SelectTrigger><SelectContent><SelectItem value="all">{t('allGatherings')}</SelectItem>{hostGatherings.map((g) => <SelectItem key={g.slug} value={g.slug}>{g.title.ar}</SelectItem>)}</SelectContent></Select></div>
